...
bool JiATR::Create(const string symbol,const ENUM_TIMEFRAMES period,const int ma_period)
  {
   ...
   m_handle=iATR(m_symbol,m_timeframe,m_ma_period);
...
//|                                                                  |
//+------------------------------------------------------------------+
bool JiBearsPower::Create(const string symbol,const ENUM_TIMEFRAMES period,
                          const int ma_period,const int applied)
  {
   m_symbol=symbol;
   m_timeframe = period;
   m_ma_period = ma_period;
   m_applied=applied;
   m_handle = iBearsPower(m_symbol,m_timeframe,m_ma_period);
   return(m_handle!=INVALID_HANDLE);
  }
//+------------------------------------------------------------------+
//|                                                                  |
...
//|                                                                  |
//+------------------------------------------------------------------+
bool JiBullsPower::Create(const string symbol,const ENUM_TIMEFRAMES period,
                          const int ma_period,const int applied)
  {
   m_symbol=symbol;
   m_timeframe = period;
   m_ma_period = ma_period;
   m_applied=applied;
   m_handle = iBullsPower(m_symbol,m_timeframe,m_ma_period);
   return(m_handle!=INVALID_HANDLE);
  }
//+------------------------------------------------------------------+
//|                                                                  |
...
//|                                                                  |
//+------------------------------------------------------------------+
bool JiCCI::Create(const string symbol,const ENUM_TIMEFRAMES period,
                   const int ma_period,const int applied)
  {
   m_symbol=symbol;
   m_timeframe = period;
   m_ma_period = ma_period;
   m_applied=applied;
   m_handle = iCCI(m_symbol,m_timeframe,m_ma_period,m_applied);
   return(m_handle!=INVALID_HANDLE);
  }
//+------------------------------------------------------------------+
//|                                                                  |
...
bool JiChaikin::Create(const string symbol,const ENUM_TIMEFRAMES period,
                       ...
                       const ENUM_MA_METHOD ma_method,const ENUM_APPLIED_VOLUME applied)
  {
   ...
   m_applied=applied;
   m_handle = iChaikin(m_symbol,m_timeframe,m_fast_ma_period,m_slow_ma_period,m_ma_method,m_applied);
   return(m_handle!=INVALID_HANDLE);
  }
//+------------------------------------------------------------------+
//|                                                                  |
...
bool JiDeMarker::Create(const string symbol,const ENUM_TIMEFRAMES period,const int ma_period)
  {
   ...
   m_timeframe = period;
   m_ma_period = ma_period;
   m_handle=iDeMarker(m_symbol,m_timeframe,m_ma_period);
   return(m_handle!=INVALID_HANDLE);
  }
//+------------------------------------------------------------------+
//|                                                                  |
...
bool JiForce::Create(const string symbol,const ENUM_TIMEFRAMES period,
                     ...
                     const ENUM_APPLIED_VOLUME applied)
  {
   ...
   m_ma_method = ma_method;
   m_applied=applied;
   m_handle = iForce(m_symbol,m_timeframe,m_ma_period,m_ma_method,(ENUM_APPLIED_VOLUME)m_applied);
   return(m_handle!=INVALID_HANDLE);
  }
//+------------------------------------------------------------------+
//|                                                                  |
...
bool JiMACD::Create(const string symbol,const ENUM_TIMEFRAMES period,
                    ...
                    const int signal_period,const int applied)
  {
   ...
   m_applied=applied;
   m_handle = iMACD(m_symbol,m_timeframe,m_fast_ema_period,m_slow_ema_period,m_signal_period,m_applied);
   return(m_handle!=INVALID_HANDLE);
  }
//+------------------------------------------------------------------+
//|                                                                  |
...
//|                                                                  |
//+------------------------------------------------------------------+
bool JiMomentum::Create(const string symbol,const ENUM_TIMEFRAMES period,
                        const int ma_period,const int applied)
  {
   m_symbol=symbol;
   m_timeframe = period;
   m_ma_period = ma_period;
   m_applied=applied;
   m_handle = iMomentum(m_symbol,m_timeframe,m_ma_period,m_applied);
   return(m_handle!=INVALID_HANDLE);
  }
//+------------------------------------------------------------------+
//|                                                                  |
...
bool JiOsMa::Create(const string symbol,const ENUM_TIMEFRAMES period,
                    ...
                    const int signal_period,const int applied)
  {
   ...
   m_signal_period=signal_period;
   m_applied=applied;
   m_handle = iOsMA(m_symbol,m_timeframe,m_fast_ema_period,m_slow_ema_period,m_signal_period,m_applied);
   return(m_handle!=INVALID_HANDLE);
  }
//+------------------------------------------------------------------+
//|                                                                  |
...
//|                                                                  |
//+------------------------------------------------------------------+
bool JiRSI::Create(const string symbol,const ENUM_TIMEFRAMES period,
                   const int ma_period,const int applied)
  {
   m_symbol=symbol;
   m_timeframe = period;
   m_ma_period = ma_period;
   m_applied=applied;
   m_handle = iRSI(m_symbol,m_timeframe,m_ma_period,m_applied);
   return(m_handle!=INVALID_HANDLE);
  }
//+------------------------------------------------------------------+
//|                                                                  |
...
bool JiRVI::Create(const string symbol,const ENUM_TIMEFRAMES period,const int ma_period)
  {
   ...
   m_timeframe = period;
   m_ma_period = ma_period;
   m_handle=iRVI(m_symbol,m_timeframe,m_ma_period);
   return(m_handle!=INVALID_HANDLE);
  }
//+------------------------------------------------------------------+
//|                                                                  |
...
bool JiStochastic::Create(const string symbol,const ENUM_TIMEFRAMES period,
                          ...
                          const ENUM_MA_METHOD ma_method,const ENUM_STO_PRICE price_field)
  {
   ...
   m_handle=iStochastic(m_symbol,m_timeframe,m_Kperiod,m_Dperiod,m_slowing,m_ma_method,m_price_field);
   return(m_handle!=INVALID_HANDLE);
  }
//+------------------------------------------------------------------+
//|                                                                  |
...
bool JiWPR::Create(const string symbol,const ENUM_TIMEFRAMES period,const int calc_period)
  {
   ...
   m_handle = iWPR(m_symbol,m_timeframe,m_calc_period);
   return(m_handle!=INVALID_HANDLE);
  }
//+------------------------------------------------------------------+
//|                                                                  |
...
//|                                                                  |
//+------------------------------------------------------------------+
bool JiTrix::Create(const string symbol,const ENUM_TIMEFRAMES period,
                    const int ma_period,const int applied)
  {
   m_symbol=symbol;
   m_timeframe=period;
   m_ma_period=ma_period;
   m_applied=applied;
   m_handle = iTriX(m_symbol,m_timeframe,m_ma_period,m_applied);
   return(m_handle!=INVALID_HANDLE);
  }
//+------------------------------------------------------------------+
//|                                                                  |
